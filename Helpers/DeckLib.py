...
def create_deck_list(cards):
    ...
    for pk in pokemon:
        text += '* ' + str(pk.count) + ' ' + pk.name.decode("UTF-8") + ' ' + pk.setName + ' ' + pk.number + '\n'

    text += '\n##Trainer Cards - ' + str(tcsum) + '\n\n'

    for tc in trainers:
        text += '* ' + str(tc.count) + ' ' + tc.name.decode("UTF-8") + ' ' + tc.setName + ' ' + tc.number + '\n'

    text += '\n##Energy - ' + str(ensum) + '\n\n'

    for en in energy:
        text += '* ' + str(en.count) + ' ' + en.name.decode("UTF-8") + '\n'

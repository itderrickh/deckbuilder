...
def get_shared_decklist(deck_one, deck_two):
	...
	deck_one_cards = [x.name + " " + x.setName for x in deck_one]
	deck_two_cards = [x.name + " " + x.setName for x in deck_two]
	# Store all the cards in the decks
	all_props = list(set(deck_one_cards) | set(deck_two_cards))

	# Go through all the properties
	for prop in all_props:
		cards_in_deck_one = [x for x in deck_one if x.name + " " + x.setName == prop]
		cards_in_deck_two = [x for x in deck_two if x.name + " " + x.setName == prop]

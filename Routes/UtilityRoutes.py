...
@util_routes.route("/api/events/hide", methods=['POST'])
@jwt_required()
...
@util_routes.route("/api/events/add", methods=['POST'])
@jwt_required()
def add_user_events():
	content = request.get_json()
	if len(ses.query(UserEvent.id).filter(UserEvent.userId==current_identity.id, UserEvent.eventId==content['eventId']).all()) <= 0:
...
@util_routes.route("/api/events/user", methods=['GET'])
@jwt_required()
...
def get_user_events(refresh="false"):
	...
	url = "https://www.pokemon.com/us/play-pokemon/pokemon-events/find-an-event/?country=176&postal_code={}&city=&event_name=&location_name=&address=&state_object=&state_other=&distance_within=100&start_date=0&end_date=90&event_type=tournament&event_type=premier&product_type=tcg&sort_order=when&results_pp=100".format(zipCode)
